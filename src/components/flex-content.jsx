export default function FlexContent({ check, heading, title, text, btn, url, img, }) {
  return (
    <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${check ? "flex-row-reverse" : "flex-row"}`}    >
      <div className="max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center">
        ...
        <a
          ...
          className="flex items-center lg:mb-5" role="button"
        >
          <button
            type="button"
            className="button-theme bg-slate-900 shadow-slate-900 text-slate-200 py-1.5"
          >
            {btn}
          </button>
        </a>
      ...
      <div className="flex items-center justify-center max-w-xl relative lg:max-w-none w-full">
        <img
          src={img} alt={title}
          className={`w-auto object-contain transitions-theme ${check
